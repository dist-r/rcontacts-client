...
// Destructure createContactMutate from hook
const { createContactMutate, isCreatingContact, createContactError } = useContact();
...
<template>
  <div class="fixed inset-0 bg-slate-950/70 backdrop-blur-sm flex items-center justify-center z-50 p-4" @click.self="closeModal">
    <div class="relative bg-slate-900/95 border border-slate-800 rounded-xl p-6 w-full max-w-md shadow-[0_0_30px_rgba(0,0,0,0.6)] overflow-hidden">
      ...
      <!-- Tech Decors -->
      <div class="absolute top-2 right-4 font-mono text-[8px] text-slate-700 select-none">NODE_ENTRY_PORT</div>
      ...
      <form @submit.prevent="handleAddContact" class="space-y-4">
        ...
        <div class="flex justify-end gap-3 pt-4 border-t border-slate-900 mt-6">
          ...
          >
            <Icon v-if="isCreatingContact" icon="line-md:loading-twotone-loop" class="text-slate-950 text-xs" />
            <span>{{ isCreatingContact ? 'COMMITING...' : 'COMMIT_RECORD' }}</span>
          </button>
        </div>
      </form>
      ...
      >
        ...
        <div>
          <div class="font-bold uppercase">COMMIT_FAILED</div>
          <div>{{ createContactError.message || createContactError }}</div>
        </div>
      </div>
    </div>
  </div>
